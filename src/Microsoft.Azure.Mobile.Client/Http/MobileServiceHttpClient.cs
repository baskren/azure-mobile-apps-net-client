...
namespace Microsoft.WindowsAzure.MobileServices
{
    internal class MobileServiceHttpClient : IDisposable
    {
        ...
        /// </summary>
        internal readonly Uri applicationUri;
        ...
        private HttpRequestMessage CreateHttpRequestMessage(HttpMethod method, string uriPathAndQuery, IDictionary<string, string> requestHeaders, HttpContent content, MobileServiceUser user)
        {
            ...
            if (MobileServiceClient.Verbose)
                System.Diagnostics.Debug.WriteLine("RequestUri: " + request.RequestUri);
                                                                 ...
                                                                 CancellationToken cancellationToken)
        {
            ...
            {
                if (MobileServiceClient.Verbose)
                    System.Diagnostics.Debug.WriteLine("RESPONSE **INVALID STATUS**: [" + response.StatusCode + "]");
            ...
            if (ensureResponseContent)
            {
                ...
                {
                    if (MobileServiceClient.Verbose)
                        System.Diagnostics.Debug.WriteLine("RESPONSE **EMPTY CONTENT**");
                    //throw new MobileServiceInvalidOperationException("The server response was empty.  ODATA content was expected.", request, response);
                }
            }
